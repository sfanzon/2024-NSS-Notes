...
\alpha \iff \beta
$$ {#eq-iff}
we mean that $\alpha$ and $\beta$ are **equivalent**. We also say that $\alpha$ is true **if and only if** $\beta$ is true. Note that (@eq-iff) is equivalent to 
$$
...
we write $A \subseteq B$ or $A \neq B$.

:::


The next proposition is very useful when we need to **prove** that two sets are equal: rather than showing directly that $A = B$, we can prove that $A \subseteq B$ and $B \subseteq A$. 
...
Hence, the power set of $\Omega$ is the set of all subsets of $\Omega$. 
...
1. $[x]$ is non-empty.
2. $[x]$ does not depend on the representative $x$: we need to check that
$$
...
*Proof of Point 1:* By the *reflexive property* we have $x \sim x$. By definition of
equivalence class we conclude that $x \in [x]$. This shows 
$$
[x] \neq \emptyset
$$
...
Assume $x \sim y$. By the definition of an equivalence class
$$
[x] = \{z \in A \mid z \sim x\}
$$ 
is the set of all elements in $A$ that are related to $x$. Similarly, 
$$
[y] = \{z \in A \mid z \sim y \} \,.
$$
We need to show that 
$$
[x] = [y] \,,
$$
meaning that every element in $[x]$ is also in $[y]$, and vice versa.

1. First, take an arbitrary element $z \in [x]$. 
...
2. Now, take an arbitrary element $z \in [y]$.
...
The prototypical (and trivial) example of equivalence relation is the **equality**
over $\Q$.
...
We now give an example of a non-trivial equivalence relation over $\Q$.
...
$${#eq-equivalence-rel}
Condition (@eq-equivalence-rel) is illustrated in @fig-equivalence-class. 
...
![For each $x \in \Q$ there exist unique $q \in [0,1) \cap \Q$ such that $x = q + n$. In particular $[x] = [q]$.](/images/equivalence_class.png){#fig-equivalence-class width=40%}
...
**Order relations** are defined similarly to **equivalence relations**. However notice that *symmetry* is replaced by
*antisymmetry*.
...
Notice that conditions 1 and 3 are the same for equivalence and order relations. Instead condition 2 changes: it is symmetry for equivalence relation, and antisymmetry for order relations.

:::


We say that a partial order is total if all the elements in the set are related.
...
The operation of **set inclusion** is a partial order
on $P(\Omega)$ but **not** a total order.
...
The **inequality** on $\Q$ is an example of total order.


::: Example
### Inequality is a total order

**Question.** Consider the binary relation induced by the inequality on $\Q \times \Q$:
$$
...
::: Notation
If $A$ is a set and $R$ is a total order on $A$, we write
$$
...
![Interval $(a,b)$](/images/interval_1.png){#fig-interval-1 width=30%}

![Interval $[a,b]$](/images/interval_2.png){#fig-interval-2 width=30%}

![Interval $(a,\infty)$](/images/interval_3.png){#fig-interval-3 width=30%}

![Interval $(-\infty,b]$](/images/interval_4.png){#fig-interval-4 width=30%}
...
$$
f(A) := \{ y \in B \divider y = f(x) \, \mbox{ for some }\, x \in A \} \subseteq B \,.
$$
:::
...
1. Define $f \ \colon A \to B$ by setting 
$$
...
![Schematic picture of the function $f$](/images/example_function_f.png){width=40%}

![Schematic picture of the function $g$](/images/example_function_g.png){width=40%}

![Schematic picture of the function $h$](/images/example_function_h.png){width=40%}


::: Example
Let us make two examples of functions on $\R$:

1. Define $f \ \colon \R \to \R$ by
$$
f(x)=x^2 \,.
$$
...
2. Define $g \ \colon \R \to \R$ as the logarithm:
$$
g(x) = \log (x) \,.
$$
...
Let us highlight some basic properties of the absolute value.



::: Proposition
### Properties of absolute value {#proposition-absolute-value-sym}
...
![Geometric interpretation of $|x|$](/images/absolute_value_1.png){#fig-absolute-value-1 width=40%}
...
Note that by Point 3 in Proposition \ref{proposition-absolute-value-sym} we have
$$
|x-y| = |y-x| \,.
$$
:::

![Geometric interpretation of $|x-y|$](/images/absolute_value_2.png){#fig-absolute-value-2 width=40%}
...
![Geometric meaning of Lemma \ref{lemma-absolute-value}](/images/absolute_value_3.png){#fig-absolute-value-3 width=40%}
...
Notice that the inequality in (@eq-triangle-inequality-bis) differs from the triangle inequality (@eq-triangle-inequality)
by a sign. Indeed it can be shown tha (@eq-triangle-inequality) and (@eq-triangle-inequality-bis) are equivalent.
...
**Solution**. To be really precise, consider the statement
$$
...
Let us compute the first 4 elements:
\begin{align}
x_1 & = 1 \\
x_2 & = \frac{x_1}{2} + 1 = 1 + \frac12 = x_1 + \frac{1}{2} \\
x_3 & = \frac{x_2}{2} + 1 = 1 + \frac12 + \frac{1}{4} = x_2 + \frac{1}{4} \\
x_4 & = \frac{x_3}{2} + 1 = 1 + \frac12 + \frac{1}{4} + \frac{1}{8} =  x_3 + \frac{1}{8}\\
\end{align}
We see a pattern: The successive term is obtained by adding a power of $1/2$ to the previous term. We therefore conjecture that
$$
...
1. We see that
$$
x_2 = 1 + \frac{1}{2} = x_1 + \frac{1}{2} 
$$
proving (@eq-induction-increasing) for $n = 1$.

2. Suppose now that (@eq-induction-increasing) holds for $n=k$.
We need to prove that (@eq-induction-increasing) holds for $n=k+1$, that is,
$$
x_{k+2} = x_{k+1} + \frac{1}{2^{k+1}} \,.
$${#eq-induction-increasing-2}
Starting with the definition of $x_{k+2}$ we see that
\begin{align*}
x_{k+2} & = \frac{x_{k+1}}{2} + 1  & (\text{definition of } x_{k+2} )  \\
        & = \frac{x_k + \frac{1}{2^k}}{2} + 1   & (\text{inductive hypothesis} ) \\
        & = \frac{x_k}{2} + 1 + \frac{1}{2^{k+1}} & (\text{just calculations} ) \\
        & = x_{k+1} + \frac{1}{2^{k+1}}  & (\text{definition of } x_{k+1}  ) \\
\end{align*}
which proves (@eq-induction-increasing-2).
