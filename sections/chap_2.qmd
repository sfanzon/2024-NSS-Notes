...
1. Equation (@eq-infinite-union) is proven using Proposition \ref{Proposition-Equality-Sets}. To this end, assume that $m \in \cup_n A_n$. Then $m \in A_n$ for at least one $n \in \N$. Since $A_n \subseteq \N$, we conclude that $m \in \N$. This shows 
$$
...
2. We prove (@eq-infinite-intersection) by **contradiction**. Indeed, suppose that (@eq-infinite-intersection) is false, i.e., 
$$
\bigcap_{n \in \N} A_n \neq \emptyset \,.
$$
...
**Solution.** $\mathcal{P}(\Omega)$ has $2^3 = 8$ elements. These are
...
$${#eq-equivalence-rel}
Condition (@eq-equivalence-rel) is illustrated in @fig-equivalence-class. Indeed: 
take $x \in \Q$ arbitrary. Then $x \in [n,n+1)$ for some 
...
![For each $x \in \Q$ there exist unique $q \in [0,1) \cap \Q$ such that $x = q + n$. In particular $[x] = [q]$.](/images/equivalence_class.png){#fig-equivalence-class width=70%}
...
Consider an arbitrary non-empty set $\Omega$ and its **power set**
$$
\mathcal{P}(\Omega) = \{ A \divider A \subseteq \Omega  \}\,.
$$
...
In general $R$ is **not** a total order. For example if we consider
$$
\Omega = \{x, y\}\,.
$$
Thus
...
& = \frac{(k+1)(k+2)}{k}
\end{align}
where in the first equality we used that (@eq-formula-sum-naturals) holds for $n=k$. We then have
$$
1 +   \ldots + k + (k+1) = \frac{(k+1)(k+2)}{k}\,,
$$
which shows that (@eq-formula-sum-naturals) holds for $n = k+1$.
