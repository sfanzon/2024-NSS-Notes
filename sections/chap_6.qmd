...
**Question.** Prove that the following sequence is bounded, and find $M$:
$$
a_n = \frac1n \,.
$$

**Solution.** In Theorem \ref{theorem-one-over-n} we have shown that 
$$
\lim_{n \to \infty} \,  \frac{1}{n} = 0 
$$
Hence, it follows from Theorem \ref{theorem-convergent-bounded} that the sequence $(1/n)$ is bounded.

An explicit constant $M$ is found as follows:
$$
\left|\frac{1}{n}\right|=\frac{1}{n} \leq 1 \,, \quad \forall \, n \in \N \,,
$$
since $n \geq 1$ for all $n \in \N$. Therefore $M=1$.
...
**Proof.** We have proven in Theorem \ref{theorem-1-minus-n} that $(a_n)$ is not convergent. However
...
*Comment.* The above is saying that no real number $M>0$ can be a bound for $|a_n|$, since there is always an index $n \in \N$ such that 
$$
|a_n| > M \,.
$$
