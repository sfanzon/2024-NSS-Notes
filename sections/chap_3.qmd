...
1. $\sqrt{2}$ is just a symbol for a number $x$ such that $x^2 = 2$. We have seen that $x \notin \Q$, which leads to the question: what is the meaning of the expression
$$
a \leq \sqrt{2} \leq b 
$$
when $a,b \in \Q$? We only know how to compare rational numbers, so the above makes no sense. We need a more clever way to write down the sets $A$ and $B$ so that they make sense as objects in $\Q$.
